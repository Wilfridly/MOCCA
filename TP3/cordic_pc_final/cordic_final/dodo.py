...
# ruleVasy  = Vasy.mkRule('vasy_init', 'cordic_top.v', "cordic_top.vhd",  flags=Vasy.UseStdLogic|Vasy.AddPowerSupplies )
ruleYosys = Yosys   .mkRule( 'yosys', 'cordic_top.v' )
ruleB2V   = Blif2Vst.mkRule( 'b2v'  , 'cordic_top.vst', [ruleYosys], flags=0 )
...
if buildChip:
    ...
    rulePnR = PnR.mkRule( 'gds'    , [ 'cordic_top_cts_r.gds'
                                     , 'cordic_top_cts_r.vst'
                                     , 'cordic_top_cts_r.spi' ]
                                     , [ruleB2V]
                                   , scriptMain
                                   , topName=topName )
    PnR.CheckLvs = False
    # ruleX2Y = x2y.mkRule( 'spi2vst', 'cordic_top_cts_r_spi.vst', 'cordic_top_cts_r.spi' )
    # ruleLvx = Lvx.mkRule( 'lvx_spi', [ 'cordic_top_cts_r.vst'
    #                                  , 'cordic_top_cts_r.spi' ]
    #                                , Lvx.MergeSupply|Lvx.Flatten )
    staLayout    = rulePnR.file_target( 2 )
    # vlogSim      = rulePnR.file_target( 1 )
    # ruleVasy     = Vasy    .mkRule( 'vasy', 'cordic_top_cts_r.v'
    #                                       , vlogSim
    #                                       , Vasy.Overwrite|Vasy.RemovePowerSupplies )
    # ruleIverilog = Iverilog.mkRule( 'iverilog', [ ruleVasy, 'gate_cordic_top_tb_cpu.v' ] )
