...
ARCHITECTURE vhd OF cordic_cor IS
    ...
    SIGNAL idx_in, idx_in_new : std_logic_vector(2 downto 0);
    SIGNAL a_p,x_p,y_p : std_logic_vector(7 downto 0);
    signal w_ready, wok_axy_ctl : std_logic;
...
-- input pipeline
    idx_in_new <= "001" when raz else
                  idx_in when wr_axy_p = '0' else
                  "010" when idx_in = "001" else
                  "100" when idx_in = "010" else
                  "001";

    w_ready <= idx_in = "100" and idx_in_new = "001";

    wok_axy_p <= (not (idx_in = idx_in_new)) and wok_axy_ctl = '1';

    a_p <= data_p when wr_axy_p = '1' and idx_in = "001" else a;
    x_p <= data_p when wr_axy_p = '1' and idx_in = "010" else x;
    y_p <= data_p when wr_axy_p = '1' and idx_in = "100" else y;
...
--     n_get       <= (get  AND NOT wr_axy_p)
    n_get       <= (get  AND NOT w_ready)
                OR (put  AND rd_nxy_p)
                ;
--     n_norm      <= (get  AND wr_axy_p)
    n_norm      <= (get  AND w_ready)
