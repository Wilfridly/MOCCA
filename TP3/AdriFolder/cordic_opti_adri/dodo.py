...
PnR.textMode       = True
pnrSuffix          = '_cts_r'
topName            = 'cordic_cor_opt'

ruleVasy  = Vasy.mkRule( 'vasy_init', 'cordic_cor_opt.v', "cordic_cor_opt.vhd", flags=Vasy.UseStdLogic|Vasy.AddPowerSupplies )
ruleYosys = Yosys   .mkRule( 'yosys', 'cordic_cor_opt.v' )
ruleB2V   = Blif2Vst.mkRule( 'b2v'  , 'cordic_cor_opt.vst', [ruleYosys], flags=0 )

if buildChip:
    TasYagle.ClockName = 'p_ck'
    # Rule for chip generation.
    ruleSeal  = SealRing.mkRule( 'sealring', targets=[ 'chip_r_seal.gds' ] , size=[1100.0, 1100.0] )
    rulePnR   = PnR.mkRule( 'gds'  , [ 'chip_r.gds'
                                     ...
                                     , 'corona.vst'
                                     , 'corona.spi'
                                     , 'cordic_cor_opt_cts.spi'
                                     , 'cordic_cor_opt_cts.vst' ]
                                     , [ruleB2V, ruleSeal]
                                   , scriptMain
                                   , topName=topName )
    staLayout = rulePnR.file_target( 6 )
else:
    TasYagle.ClockName = 'ck'
    # Rule for block generation.
    rulePnR = PnR.mkRule( 'gds'    , [ 'cordic_cor_opt_cts_r.gds'
                                     , 'cordic_cor_opt_cts_r.vst'
                                     , 'cordic_cor_opt_cts_r.spi'
                                     ]
                                     , [ruleB2V]
                                   , scriptMain
                                   , topName=topName )
    PnR.CheckLvs = False
#    ruleX2Y = x2y.mkRule( 'spi2vst', 'cordic_cor_opt_cts_r_spi.vst', 'cordic_cor_opt_cts_r.spi' )
#    ruleLvx = Lvx.mkRule( 'lvx_spi', [ 'cordic_cor_opt_cts_r.vst', 'cordic_cor_opt_cts_r.spi' ]
#                                   , Lvx.MergeSupply|Lvx.Flatten )
#    staLayout = rulePnR.file_target( 2 )
#    vlogSim   = rulePnR.file_target( 1 )
#    ruleVasy  = Vasy.mkRule( 'vasy', 'cordic_cor_opt_cts_r.v', vlogSim, Vasy.Overwrite|Vasy.RemovePowerSupplies )
