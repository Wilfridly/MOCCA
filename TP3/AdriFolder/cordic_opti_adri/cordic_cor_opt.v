module cordic_cor_opt (ck, raz, wr_axy_p, data_p, wok_axy_p, rd_nxy_p, nx_p, ny_p, rok_nxy_p, vdd, vss);
  ...
  wire	  reg_def_49;
  wire	  fsm_def_47;
  assign	rtlexts_4 = a[7:0];
  assign	rtlexts_3 = x[7:0];
  assign	rtlexts_2 = y[7:0];
  ...
  always @ ( posedge ck )
    begin
      calc = (rtldef_4 & n_calc);
    end

  always @ ( posedge ck )
    begin
      norm = (rtldef_3 & n_norm);
    end

  always @ ( posedge ck )
    begin
      get = ((rtldef_2 & n_get) | fsm_def_47);
    end

  always @ ( fsm_def_47 )
    if (fsm_def_47 == 1'b0) rtldef_7 = 1'b1;
    else rtldef_7 = 1'b0;

  always @ ( fsm_def_47 )
    if (fsm_def_47 == 1'b0) rtldef_6 = 1'b1;
    else rtldef_6 = 1'b0;
  ...
  assign	n_norm = ((get & w_ready) | (norm & ~(quadrant_0)));
  assign	n_get = ((get & ~(w_ready)) | (put & rd_nxy_p));

  always @ ( posedge ck )
    begin
      idx_in = (({rtldef_0 , rtldef_0 , rtldef_0} & idx_in_new) | ({rtldef_1 , rtldef_1 , rtldef_1
} & 3'b001));
    end

  always @ ( reg_def_49 )
    if (reg_def_49 == 1'b1) rtldef_1 = 1'b1;
    else rtldef_1 = 1'b0;

  always @ ( reg_def_49 )
    if (reg_def_49 == 1'b0) rtldef_0 = 1'b1;
    else rtldef_0 = 1'b0;
  assign	reg_def_49 = (raz == 1'b0);

  always @ ( rtlexts_2 or data_p or idx_in or wr_axy_p )
    if (((wr_axy_p == 1'b1) & (idx_in == 3'b100)) == 1'b1) y_p = data_p;
    else y_p = rtlexts_2;

  always @ ( rtlexts_3 or data_p or idx_in or wr_axy_p )
    if (((wr_axy_p == 1'b1) & (idx_in == 3'b010)) == 1'b1) x_p = data_p;
    else x_p = rtlexts_3;

  always @ ( rtlexts_4 or data_p or idx_in or wr_axy_p )
    if (((wr_axy_p == 1'b1) & (idx_in == 3'b001)) == 1'b1) a_p = data_p;
    else a_p = rtlexts_4;
  assign	wok_axy_p = (~((idx_in == idx_in_new)) & (wok_axy_ctl == 1'b1));
  assign	w_ready = ((idx_in == 3'b100) & (idx_in_new == 3'b001));

  always @ ( idx_in or wr_axy_p or raz )
    if (raz == 1'b1) idx_in_new = 3'b001;
    else if (wr_axy_p == 1'b0) idx_in_new = idx_in;
    else if (idx_in == 3'b001) idx_in_new = 3'b010;
    else if (idx_in == 3'b010) idx_in_new = 3'b100;
    else idx_in_new = 3'b001;
