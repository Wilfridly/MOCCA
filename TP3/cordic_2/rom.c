...
int main( int argc, char * argv[]) {
    ...
    unsigned x_tab[1 << ADDRWDMAX];
    unsigned y_tab[1 << ADDRWDMAX];
    unsigned char nx_tab[1 << ADDRWDMAX];
    unsigned char ny_tab[1 << ADDRWDMAX];

    unsigned rangelen = 1+(valwd-1)/4;
    char *name = "a_in";
    unsigned namelen = strlen(name);
    char *name4 = "x_in";
    unsigned namelen4 = strlen(name4);
    char *name5 = "y_in";
    unsigned namelen5 = strlen(name5);
    char *name2 = "nx_in";
    ...
    for(int i=0; i < valuenb; i++) {
        if (i==0) 
            ...
            printf ("%*s x\"%0*x\" when pt = %d\n", namelen, "else", rangelen, a_tab[i], i);
    }
    printf ("%*s x\"%0*x\";\n", namelen, "else", rangelen, 0);
    ...
    for(int i=0; i < valuenb; i++) {
        if (i==0) 
            ...
            printf ("%*s x\"%0*x\" when pt = %d\n", namelen2, "else", rangelen, nx_tab[i], i);
    }
    printf ("%*s x\"%0*x\";\n", namelen2, "else", rangelen, 0);
    ...
    for(int i=0; i < valuenb; i++) {
        if (i==0) 
            ...
            printf ("%*s x\"%0*x\" when pt = %d\n", namelen3, "else", rangelen, ny_tab[i], i);
    }
    printf ("%*s x\"%0*x\";\n", namelen3, "else", rangelen, 0);

    printf("\n");
    for(int i=0; i < valuenb; i++) {
        if (i==0) 
            printf ("%*s <= x\"%0*x\" when pt = %d\n", namelen4, name4, rangelen, x_tab[i], i);
        else
            printf ("%*s x\"%0*x\" when pt = %d\n", namelen4, "else", rangelen, x_tab[i], i);
    }
    printf ("%*s x\"%0*x\";\n", namelen4, "else", rangelen, 0);

    printf("\n");
    for(int i=0; i < valuenb; i++) {
        if (i==0) 
            printf ("%*s <= x\"%0*x\" when pt = %d\n", namelen5, name5, rangelen, y_tab[i], i);
        else
            printf ("%*s x\"%0*x\" when pt = %d\n", namelen5, "else", rangelen, y_tab[i], i);
    }
    printf ("%*s x\"%0*x\";\n", namelen5, "else", rangelen, 0);
    return 0;
}